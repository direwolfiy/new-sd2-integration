    ...
    <div className="rounded-xl border border-white/[0.12] bg-[#181818] p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          {editingNameId === state.id ? (
            <input ref={nameInputRef} value={nameDraft} onChange={(e) => setNameDraft(e.target.value)} onBlur={onSaveName} onKeyDown={(e) => { if (e.key === "Enter") onSaveName(); if (e.key === "Escape") setEditingNameId(null); }} className="h-7 px-2 rounded-md bg-[#2b2b2b] border border-white/[0.1] text-[14px] text-white font-medium outline-none focus:border-[#00CAE0]/50 w-32" />
          ) : (
          ...
          <span className="text-[12px] text-[#a3a3a3]">{state.images.length} 张</span>
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={() => onOpenStateModal(state)} className="h-7 px-2.5 rounded-full bg-white/[0.10] text-[12px] text-[#b8b8b8] flex items-center gap-1 hover:bg-white/[0.1] hover:text-white transition-colors duration-200"><Pencil size={11} strokeWidth={1.5} />编辑信息</button>
          ...
          <div className="relative">
            <button onClick={() => onSetMoreMenuId(moreMenuId === state.id ? null : state.id)} className="w-7 h-7 rounded-full flex items-center justify-center text-[#a3a3a3] hover:text-white hover:bg-white/[0.10] transition-colors duration-200"><MoreHorizontal size={16} strokeWidth={1.5} /></button>
            {moreMenuId === state.id && (
              <div className="absolute right-0 top-full mt-1 w-32 py-1 rounded-lg border border-white/[0.14] bg-[#1c1c1c] shadow-[0_8px_24px_rgba(0,0,0,0.5)] z-10">
                <button onClick={() => onDeleteState(state.id)} className="w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-[#ef4444] hover:bg-white/[0.10] transition-colors duration-200"><Trash2 size={14} strokeWidth={1.5} />删除状态</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex gap-4">
        {state.images.map((img) => {
          ...
          return (
            <div key={img.id} className="group relative w-[240px] shrink-0 rounded-lg overflow-hidden border border-white/[0.12] bg-[#0a0a0a] transition-all duration-200 hover:shadow-[0_0_0_1px_rgba(255,255,255,0.12)]">
              <div onClick={() => onOpenGenerate(state)} className="relative aspect-video bg-gradient-to-br from-[#1a1a1a] to-[#141414] flex items-center justify-center cursor-pointer">
                ...
                {img.isPrimary ? (
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onSetPrimary(state.id, img.id); }} className="absolute top-2 right-2 w-5 h-5 rounded-full bg-white/[0.10] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-white/[0.12]"><Star size={10} strokeWidth={1.5} className="text-[#a3a3a3] group-hover:text-[#b8b8b8]" /></button>
                )}
              </div>
              <div className="px-2 py-1.5 border-t border-white/[0.10]">
                {isEditingImg ? (
                  ...
                  <button onClick={() => onStartEditImageName(state.id, img.id, img.name)} className="w-full text-left text-[11px] text-[#b8b8b8] hover:text-white transition-colors duration-200 truncate">{img.name}</button>
                )}
              </div>
            </div>
          );
