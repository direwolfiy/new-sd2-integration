      ...
      <div className="flex flex-1 min-h-0">
        <nav className="w-[160px] shrink-0 border-r border-white/[0.06] p-3 overflow-y-auto">
          <div className="flex flex-col gap-3">
            {scenes.map((scene) => (
              <button key={scene.id} onClick={() => setSelectedId(String(scene.id))} className={`shrink-0 rounded-lg overflow-hidden transition-all duration-200 ${String(scene.id) === selectedId ? "ring-2 ring-white/20 bg-white/[0.04]" : "opacity-50 hover:opacity-80"}`}>
          ...
          <div className="p-6 space-y-4">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    {selected.template_category && <span className="px-2 py-0.5 rounded-full bg-white/[0.06] text-[12px] text-[#999]">{selected.template_category}</span>}
                  </div>
                </div>
              </div>
              <div className="mt-4">
                <span className="text-[12px] text-[#666]">场景描述</span>
            ...
            {selected.appearance && typeof selected.appearance === "object" && (
              <div className="rounded-xl border border-white/[0.06] p-4">
                ...
                <pre className="text-[12px] text-[#999] leading-[1.7] whitespace-pre-wrap break-words">{JSON.stringify(selected.appearance, null, 2)}</pre>
              </div>
            )}
          </div>
        </div>
