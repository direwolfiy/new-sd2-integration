    ...
    <div className="rounded-xl border border-white/[0.06] bg-[#141414] p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] text-[#666]">{variant.images.length} 张</span>
        </div>

        <div className="flex items-center gap-1.5">
          <button onClick={() => onOpenVariantModal(variant)} className="h-7 px-2.5 rounded-full bg-white/[0.06] text-[12px] text-[#999] flex items-center gap-1 hover:bg-white/[0.1] hover:text-white transition-colors duration-200">
          ...
          <div className="relative">
            <button onClick={() => onSetMoreMenuId(moreMenuId === variant.id ? null : variant.id)} className="w-7 h-7 rounded-full flex items-center justify-center text-[#666] hover:text-white hover:bg-white/[0.06] transition-colors duration-200">
            ...
            {moreMenuId === variant.id && (
              <div className="absolute right-0 top-full mt-1 w-32 py-1 rounded-lg border border-white/[0.08] bg-[#1c1c1c] shadow-[0_8px_24px_rgba(0,0,0,0.5)] z-10">
                <button onClick={() => onDeleteVariant(variant.id)} className="w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-[#ef4444] hover:bg-white/[0.06] transition-colors duration-200">
      ...
      <div className="flex gap-4">
        {variant.images.map((img) => {
          ...
          return (
            <div key={img.id} className="group relative w-[140px] shrink-0 rounded-lg overflow-hidden border border-white/[0.06] bg-[#0a0a0a] transition-all duration-200 hover:shadow-[0_0_0_1px_rgba(255,255,255,0.12)]">
              <div onClick={() => onOpenGenerate(variant)} className="relative aspect-[3/4] bg-gradient-to-br from-[#1a1a1a] to-[#141414] flex items-center justify-center cursor-pointer">
                ...
                {img.isPrimary ? (
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onSetPrimary(variant.id, img.id); }} className="absolute top-2 right-2 w-5 h-5 rounded-full bg-white/[0.06] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-white/[0.12]">
              ...
              <div className="px-2 py-1.5 border-t border-white/[0.04]">
                {isEditingImg ? (
                  ...
                  <button onClick={() => onStartEditImageName(variant.id, img.id, img.name)} className="w-full text-left text-[11px] text-[#999] hover:text-white transition-colors duration-200 truncate">
                    {img.name}
                  </button>
                )}
              </div>
