...
export function CreditInsufficientModal({ open, onClose, required, current }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-[400px] rounded-xl border border-white/[0.14] bg-[#1c1c1c] shadow-[0_16px_48px_rgba(0,0,0,0.5)]">
        ...
        <div className="px-5 py-6">
          <div className="flex items-center justify-center gap-8 mb-6">
            <div className="text-center">
              <p className="text-[12px] text-[#a3a3a3] mb-1">当前余额</p>
              <p className="text-[24px] font-medium tabular-nums">{current.toLocaleString()}</p>
            </div>
            <ArrowRight size={20} strokeWidth={1.5} className="text-[#888]" />
            <div className="text-center">
              <p className="text-[12px] text-[#a3a3a3] mb-1">所需积分</p>
              <p className="text-[24px] font-medium text-[#00CAE0] tabular-nums">{required.toLocaleString()}</p>
            </div>
          </div>

          <div className="rounded-lg bg-[#2b2b2b] px-4 py-3 mb-5">
            <p className="text-[13px] text-[#b8b8b8] leading-[1.6]">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="flex-1 h-10 rounded-full bg-white/[0.10] text-[13px] text-[#b8b8b8] hover:bg-white/[0.1] hover:text-white transition-colors duration-200"
            >
              取消
            </button>
