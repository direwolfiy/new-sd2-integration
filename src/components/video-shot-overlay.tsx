...
function Dropdown({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute bottom-full left-0 mb-1 py-1 rounded-lg border border-white/[0.14] bg-[#1c1c1c] shadow-[0_8px_24px_rgba(0,0,0,0.5)] z-10 min-w-[80px]">
          {options.map((opt) => (
            <button
              key={opt}
              onClick={() => { onChange(opt); setOpen(false); }}
              className={`w-full text-left px-3 py-1.5 text-[12px] transition-colors duration-200 ${
                opt === value ? "text-white bg-white/[0.10]" : "text-[#b8b8b8] hover:text-white hover:bg-white/[0.08]"
              }`}
            >
              {opt}
            </button>
...
export function VideoShotOverlay({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0a0a0a] flex flex-col">
      {/* Header */}
      <div className="shrink-0 flex items-center gap-4 px-5 py-3 border-b border-white/[0.12]">
        <button
          onClick={onClose}
          className="flex items-center gap-2 text-[13px] text-[#b8b8b8] hover:text-white transition-colors duration-200 shrink-0"
        >
          <ArrowLeft size={16} strokeWidth={1.5} />
          返回
        </button>

        <div className="w-px h-6 bg-white/[0.10] shrink-0" />
        ...
        <div className="flex items-center gap-2.5 flex-1 min-w-0 justify-center py-2">
          {visibleShots.map((shot) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-16 h-10 rounded-md bg-[#2b2b2b] flex items-center justify-center transition-all duration-200 ${
                ...
                >
                  {shot.hasImage ? (
                    ...
                    <span className="text-[9px] text-[#888]">#{shot.number}</span>
                  )}
                </div>
                <span className={`text-[10px] ${isActive ? "text-white" : "text-[#a3a3a3]"}`}>
                  镜头 {shot.number}
                </span>
              </button>
            );
        ...
        {shots.length > maxVisible && (
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              className="w-7 h-7 rounded-md flex items-center justify-center text-[#a3a3a3] hover:text-white hover:bg-white/[0.10] disabled:opacity-30 disabled:cursor-default transition-colors duration-200"
            >
      ...
      <div className="flex flex-1 min-h-0 p-4 gap-4">
            ...
            <div className="px-4 pt-4 pb-3 border-b border-white/[0.12] shrink-0">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {[1, 2].map((n) => (
                  <div
                    key={n}
                    className="w-20 h-20 rounded-lg bg-[#2b2b2b] border border-white/[0.12] flex items-center justify-center"
                  >
                    <ImageIcon size={16} strokeWidth={1.5} className="text-[#888]" />
                  </div>
                ))}
                <button className="w-20 h-20 rounded-lg border border-dashed border-white/[0.1] flex items-center justify-center text-[#a3a3a3] hover:border-white/[0.2] hover:text-[#b8b8b8] transition-colors duration-200">
        ...
        <div className="flex-1 rounded-xl border border-white/[0.12] bg-[#181818] flex flex-col min-w-0 overflow-hidden">
          <div className="flex items-center justify-between px-5 pt-5 pb-2 shrink-0">
            ...
            {hasHistory && (
              <span className="text-[12px] text-[#a3a3a3]">{versions.length} 个版本</span>
            )}
          </div>
          ...
          {versionsLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <div className="w-12 h-12 rounded-xl bg-white/[0.08] flex items-center justify-center mx-auto mb-3">
                  <Play size={24} strokeWidth={1.5} className="text-[#a3a3a3]" />
                </div>
                <p className="text-[13px] text-[#a3a3a3] mb-1">尚无生成记录</p>
                <p className="text-[12px] text-[#888]">在左侧配置参数后点击生成</p>
              </div>
            </div>
          ) : (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between px-1">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          <span className="text-[11px] text-[#b8b8b8]">{v.duration}</span>
                      ...
                      {/* Prompt */}
                      <p className="text-[12px] text-[#b8b8b8] leading-[1.6] px-1 line-clamp-2">
                        {v.prompt}
                      </p>
                      {/* Video preview */}
                      <div className={`aspect-video rounded-lg overflow-hidden border flex items-center justify-center relative transition-colors duration-200 ${
                        isApplied
                          ? "border-[#00CAE0]/30 bg-[#202020]"
                          : "border-white/[0.12] bg-[#202020]"
                      }`}>
                        {v.status === "generating" ? (
                          ...
                          <span className="text-[12px] text-[#888]">等待生成</span>
                        )}
                      </div>
                      {/* Actions */}
                      {v.status === "completed" && (
                        <div className="flex items-center justify-between px-1">
                          <button
                            ...
                            className={`h-7 px-3 rounded-full text-[11px] flex items-center gap-1.5 transition-colors duration-200 ${
                              isApplied
                                ...
                                : "bg-white/[0.10] text-[#b8b8b8] hover:bg-[rgba(0,202,224,0.08)] hover:text-[#00CAE0]"
                            }`}
                          >
                            {isApplied ? (
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <button className="w-7 h-7 rounded-md flex items-center justify-center text-[#a3a3a3] hover:text-[#ef4444] hover:bg-white/[0.10] transition-colors duration-200">
                  ...
                  {versions.map((v) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full aspect-video rounded-md overflow-hidden relative transition-all duration-200 bg-[#202020] hover:opacity-100 opacity-60"
                      >
