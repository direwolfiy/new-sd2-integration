...
export function DeleteConfirmModal({ open, onConfirm, onCancel }: DeleteConfirmModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-[360px] rounded-xl border border-white/[0.14] bg-[#1c1c1c] shadow-[0_16px_48px_rgba(0,0,0,0.5)]">
        <div className="p-5">
          ...
          <p className="text-[13px] text-[#b8b8b8] leading-[1.7]">确定要删除该元素吗？此操作无法撤销。</p>
        </div>
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-white/[0.12]">
          <button onClick={onCancel} className="h-9 px-4 rounded-full bg-white/[0.10] text-[13px] text-[#b8b8b8] hover:bg-white/[0.1] hover:text-white transition-colors duration-200">取消</button>
