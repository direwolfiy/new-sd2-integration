  ...
  return (
    <div className="mt-3 flex items-center gap-2">
      <div className="flex items-center gap-1 flex-1 flex-wrap">
        <button onClick={() => onTypeChange("IMAGE")} className={`h-7 px-2.5 rounded-full text-[12px] font-medium flex items-center gap-1.5 transition-colors duration-200 ${type === "IMAGE" ? "bg-[rgba(0,202,224,0.08)] text-[rgba(0,202,224,0.8)] border border-[rgba(0,202,224,0.15)]" : "bg-white/[0.06] text-[#999] border border-transparent hover:bg-white/[0.1]"}`}>
          <Image size={13} strokeWidth={1.5} />图片
        </button>
        <button onClick={() => onTypeChange("VIDEO")} className={`h-7 px-2.5 rounded-full text-[12px] font-medium flex items-center gap-1.5 transition-colors duration-200 ${type === "VIDEO" ? "bg-[rgba(0,202,224,0.08)] text-[rgba(0,202,224,0.8)] border border-[rgba(0,202,224,0.15)]" : "bg-white/[0.06] text-[#999] border border-transparent hover:bg-white/[0.1]"}`}>
        ...
        {type === "IMAGE" ? (
          <>
            <button className="h-7 px-2.5 rounded-full bg-white/[0.06] text-[12px] text-[#999] flex items-center gap-1 hover:bg-white/[0.1] transition-colors duration-200"><Frame size={13} strokeWidth={1.5} />2K</button>
            <button className="h-7 px-2.5 rounded-full bg-white/[0.06] text-[12px] text-[#999] flex items-center gap-1 hover:bg-white/[0.1] transition-colors duration-200"><Crop size={13} strokeWidth={1.5} />16:9</button>
          </>
        ) : (
          <>
            <button className="h-7 px-2.5 rounded-full bg-white/[0.06] text-[12px] text-[#999] flex items-center gap-1 hover:bg-white/[0.1] transition-colors duration-200"><Clock size={13} strokeWidth={1.5} />5秒</button>
            <button className="h-7 px-2.5 rounded-full bg-white/[0.06] text-[12px] text-[#999] flex items-center gap-1 hover:bg-white/[0.1] transition-colors duration-200"><Crop size={13} strokeWidth={1.5} />16:9</button>
            <button className="h-7 px-2.5 rounded-full bg-white/[0.06] text-[12px] text-[#999] flex items-center gap-1 hover:bg-white/[0.1] transition-colors duration-200"><Volume2 size={13} strokeWidth={1.5} />有声</button>
          </>
        )}
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <span className="text-[11px] text-[#666]">{promptLength}/4000</span>
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-[640px] h-[75vh] rounded-xl border border-white/[0.08] bg-[#1c1c1c] shadow-[0_16px_48px_rgba(0,0,0,0.5)] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] shrink-0">
          ...
          <button onClick={closePicker} className="w-7 h-7 rounded-full flex items-center justify-center text-[#666] hover:text-white hover:bg-white/[0.06] transition-colors duration-200">
        ...
        <div className="flex gap-1 p-1 mx-5 mt-4 rounded-full bg-white/[0.06] shrink-0">
          {sourceTabs.map(({ key, label, icon: Icon }) => (
            <button key={key} onClick={() => { setActiveTab(key); setRefSearch(""); }} className={`flex-1 h-8 rounded-full text-[12px] flex items-center justify-center gap-1.5 transition-all duration-200 ${activeTab === key ? "bg-white/10 text-white" : "text-[#999] hover:text-white"}`}>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5">
          {activeTab === "asset" && (
            <>
              <div className="flex items-center h-8 px-3 rounded-full bg-[#262626] gap-2 text-[12px] text-[#666] mb-4">
                ...
                <input value={refSearch} onChange={(e) => setRefSearch(e.target.value)} placeholder="搜索资产..." className="flex-1 bg-transparent outline-none text-[12px] text-white placeholder:text-[#666]" />
              </div>
              <div className="grid grid-cols-4 gap-3">
                {filteredAssets.map((asset) => {
                  ...
                  return (
                    <button key={asset.id} onClick={() => !attached && toggleTempSelect(asset.id)} className={`rounded-lg overflow-hidden border transition-all duration-200 ${attached ? "border-[rgba(0,202,224,0.2)] opacity-50 cursor-default" : selected ? "border-[rgba(0,202,224,0.4)] ring-1 ring-[rgba(0,202,224,0.3)]" : "border-white/[0.06] hover:shadow-[0_0_0_1px_rgba(255,255,255,0.12)]"}`}>
          ...
          {activeTab === "local" && (
            <div onClick={() => fileInputRef.current?.click()} onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); }} onDrop={(e) => { e.preventDefault(); e.stopPropagation(); if (e.dataTransfer.files.length) { handleLocalFiles(e.dataTransfer.files); closePicker(); } }} className="flex flex-col items-center justify-center py-16 border-2 border-dashed border-white/[0.1] rounded-xl hover:border-white/[0.2] transition-colors duration-200 cursor-pointer">
              <div className="w-12 h-12 rounded-xl bg-white/[0.04] flex items-center justify-center mb-4"><Upload size={24} strokeWidth={1.5} className="text-[#666]" /></div>
          ...
          {activeTab === "history" && (
            <div className="flex flex-col gap-2">
              {completedTasks.map((task) => {
                ...
                return (
                  <button key={task.id} onClick={() => !attached && toggleTempSelect(refId)} className={`w-full flex items-center gap-3 p-3 rounded-lg text-left transition-all duration-200 ${attached ? "bg-[rgba(0,202,224,0.05)] border border-[rgba(0,202,224,0.15)] opacity-50 cursor-default" : selected ? "bg-[rgba(0,202,224,0.05)] border border-[rgba(0,202,224,0.2)]" : "border border-white/[0.06] hover:border-white/[0.12]"}`}>
                    <div className="w-12 h-12 rounded-lg bg-[#262626] border border-white/[0.06] flex items-center justify-center shrink-0">
                      {task.type === "VIDEO" ? <Film size={16} strokeWidth={1.5} className="text-[#666]" /> : <Image size={16} strokeWidth={1.5} className="text-[#666]" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] leading-[1.5] truncate">{task.prompt}</p>
                      <p className="text-[11px] text-[#666] mt-0.5">{task.createdAt} · {task.model}</p>
                    </div>
                    {selected || attached ? (
          ...
          {activeTab === "inspiration" && (
            <div className="flex flex-col items-center justify-center py-16">
              <div className="w-12 h-12 rounded-xl bg-white/[0.04] flex items-center justify-center mb-4"><Sparkles size={24} strokeWidth={1.5} className="text-[#444]" /></div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.06] shrink-0">
          <span className="text-[12px] text-[#666]">{tempSelected.size > 0 ? `已选择 ${tempSelected.size} 张` : "点击图片选择"}</span>
          <div className="flex items-center gap-2">
            <button onClick={closePicker} className="h-9 px-4 rounded-full bg-white/[0.06] text-[13px] text-[#999] hover:bg-white/[0.1] hover:text-white transition-colors duration-200">取消</button>
